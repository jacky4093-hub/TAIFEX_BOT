...
class TaifexBotGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("TAIFEX Bot V3 - 保證金風控模擬交易")
    ...
    def build_ui(self):
        ...
        title = QLabel("台指期多商品多策略模擬交易機器人 V3")
        ...
        account.addWidget(QLabel("帳戶權益數"), 0, 2)
        account.addWidget(self.initial_equity_spin, 0, 3)
        account.addWidget(QLabel("口數"), 0, 4)
        account.addWidget(self.contracts_spin, 0, 5)
        account.addWidget(QLabel("單口保證金"), 0, 6)
        account.addWidget(self.margin_spin, 0, 7)
