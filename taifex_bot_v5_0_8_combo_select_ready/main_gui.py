...
APP_VERSION = load_local_version().get("version", "5.0.8")


class FixedComboBox(QComboBox):
    """讓下拉選單在 Linux/WSL/PyQt 環境中，點選後立即套用並收合。"""
    ...
    def _on_item_pressed(self, index):
        ...
        QTimer.singleShot(0, self._finish_select)
...
class TaifexBotGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"TAIFEX Bot V5-0-8 - 下拉選單完整修正版")
    ...
    def build_ui(self):
        ...
        title = QLabel(f"台指期多商品多策略模擬交易機器人 V5-0-8  版本：{APP_VERSION}")
