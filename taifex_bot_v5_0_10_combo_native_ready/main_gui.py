...
APP_VERSION = load_local_version().get("version", "5.0.10")


class FixedComboBox(QComboBox):
    """
    使用 PyQt6 原生 QComboBox 行為。

    前幾版為了強制收合下拉選單，攔截了 popup 點選事件，
    反而造成 WSL/Linux 環境下選單無法正常收合或無法再次選擇。
    這版移除事件攔截，讓 QComboBox 回到正常行為：
    點選項目 -> 立即選定 -> 自動收合 -> 下次可重新展開選擇。
    """
    pass
...
class TaifexBotGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"TAIFEX Bot V5-0-10 - 下拉選單原生修正版")
    ...
    def build_ui(self):
        ...
        title = QLabel(f"台指期多商品多策略模擬交易機器人 V5-0-10  版本：{APP_VERSION}")
