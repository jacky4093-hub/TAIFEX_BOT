...
APP_VERSION = load_local_version().get("version", "4.0.0")

class TaifexBotGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"TAIFEX Bot V5-0 - 行情與交易介面解耦版")
    ...
    def build_ui(self):
        ...
        title = QLabel(f"台指期多商品多策略模擬交易機器人 V5-0  版本：{APP_VERSION}")
    ...
    def reset_system(self):
        ...
        self.market = MarketFactory.create(mode="sim", start_price=self.current_price)
        self.strategy.reset()
        self.trader = BrokerFactory.create(
            mode="sim",
            initial_equity=self.initial_equity_spin.value(),
            point_value=info["point_value"],
            margin_per_contract=self.margin_spin.value(),
            contracts=self.contracts_spin.value(),
        )
        self.log.clear()
        self.append_log("系統已重置，目前使用 MarketFactory 建立模擬行情")
        self.append_log("目前使用 BrokerFactory 建立模擬交易，不會真實下單")
        self.append_log("Shioaji 真實行情與真實下單皆已保留介面，預設鎖定")
        self.update_labels()
    ...
    def start(self):
        if self.market_source_box.currentText().startswith("Shioaji"):
            QMessageBox.information(
                self,
                "尚未啟用",
                "Shioaji 真實行情目前只是預留。\nV5-0 已完成行情介面解耦，等你申請 API Key 後再接 shioaji_market.py。"
            )
            return
