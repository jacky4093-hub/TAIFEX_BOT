...
APP_VERSION = load_local_version().get("version", "5.0.13")


class FixedComboBox(QWidget):
    """
    內嵌式選單，不使用 QComboBox / QMenu popup。

    原本在 WSLg / Windows 桌面環境，Qt 的 popup 下拉視窗可能不收合、殘留在桌面，
    所以這版改成「按鈕 + 內嵌選項區塊」。
    點一下打開，再點選項會立即套用並收合；下一次可重新選其他選項。
    """
    ...
    def __init__(self, parent=None):
        ...
        self._button.clicked.connect(self.toggle_options)
        self._main_layout.addWidget(self._button)

        self._panel = QFrame()
        self._panel.setFrameShape(QFrame.Shape.Box)
    ...
    def addItem(self, text):
        ...
        btn.clicked.connect(lambda checked=False, value=text: self.select_item(value))
...
class TaifexBotGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"TAIFEX Bot V5-0-13 - 內嵌選單修正版")
    ...
    def build_ui(self):
        ...
        title = QLabel(f"台指期多商品多策略模擬交易機器人 V5-0-13  版本：{APP_VERSION}")
