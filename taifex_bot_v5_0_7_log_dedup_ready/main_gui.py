...
APP_VERSION = load_local_version().get("version", "5.0.0")

class TaifexBotGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"TAIFEX Bot V5-0-7 - 日誌防洗版")
    ...
    def build_ui(self):
        ...
        title = QLabel(f"台指期多商品多策略模擬交易機器人 V5-0-7  版本：{APP_VERSION}")
        ...
        self.market_source_box = QComboBox()
        ...
        self.max_loss_unit_box = QComboBox()
        ...
        for combo in [
            self.contract_box,
            self.market_source_box,
            self.breakout_upper_action,
            self.breakout_lower_action,
            self.max_loss_unit_box,
        ]:
            self.fix_combo_behavior(combo)

        self.setLayout(main)


    def fix_combo_behavior(self, combo):
        """修正部分 Linux/WSL/PyQt 環境下 QComboBox 點選後選單殘留的問題。"""
        combo.activated.connect(lambda _index, c=combo: self.on_combo_selected(c))

    def on_combo_selected(self, combo):
        combo.hidePopup()
        combo.clearFocus()
        self.setFocus()
