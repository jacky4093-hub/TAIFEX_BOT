...
class TaifexBotGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"TAIFEX Bot V5-0-2 - 突破策略方向版")
    ...
    def build_ui(self):
        ...
        title = QLabel(f"台指期多商品多策略模擬交易機器人 V5-0-2  版本：{APP_VERSION}")
        ...
        grid.addWidget(self.enable_grid, 4, 0)
        grid.addWidget(QLabel("網格基準"), 4, 1)
        grid.addWidget(self.grid_base, 4, 2)
        grid.addWidget(QLabel("網格間距"), 4, 3)
        grid.addWidget(self.grid_size, 4, 4)
